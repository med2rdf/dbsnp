        ...
        context 'for three_snps.vcf' do
          ...
          context 'for rs775809821' do
            ...
            it 'should have rdf triples' do
              ...
              faldo_nodes = subject.select do |statement|
                statement.subject == refsnp_uri && statement.predicate == Faldo.location
              end.map(&:object)
              ...
              expect(faldo_nodes.any? do |faldo_node|
                begin_subject = subject.select do |statement|
                  statement.subject == faldo_node && statement.predicate == Faldo.begin
                end[0].object

                end_subject = subject.select do |statement|
                  statement.subject == faldo_node && statement.predicate == Faldo.end
                end[0].object
                subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.Region)) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.begin, begin_subject)) &&
                    subject.include?(RDF::Statement.new(begin_subject, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(begin_subject, Faldo.position, 10019)) &&
                    subject.include?(RDF::Statement.new(begin_subject, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10'))) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.end, end_subject)) &&
                    subject.include?(RDF::Statement.new(end_subject, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(end_subject, Faldo.position, 10020)) &&
                    subject.include?(RDF::Statement.new(end_subject, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10')))
              end
              ).to eq(true)
            end
          end
          ...
          context 'for rs978760828' do
            ...
            it 'should have rdf triples' do
              ...
              expect(faldo_nodes.any? do |faldo_node|
                subject.include?(RDF::Statement.new(faldo_node, Faldo.position, 10039)) &&
                    subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10')))
              end
              ).to eq(true)
          ...
          context 'for rs1008829651' do
            ...
            it 'should have rdf triples' do
              ...
              expect(faldo_nodes.any? do |faldo_node|
                subject.include?(RDF::Statement.new(faldo_node, Faldo.position, 10043)) &&
                    subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10')))
              end
              ).to eq(true)
        ...
        context 'for single line vcfs' do
            ...
            it 'should have rdf triples' do
              ...
              expect(faldo_nodes.any? do |faldo_node|
                ...
                subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.Region)) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.begin, begin_subject)) &&
                    subject.include?(RDF::Statement.new(begin_subject, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(begin_subject, Faldo.position, 10228)) &&
                    subject.include?(RDF::Statement.new(begin_subject, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.11'))) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.end, end_subject)) &&
                    subject.include?(RDF::Statement.new(end_subject, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(end_subject, Faldo.position, 10255)) &&
                    subject.include?(RDF::Statement.new(end_subject, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.11')))
              end
              ).to eq(true)
            end
          end
          ...
          context 'for rs71286109.vcf' do
            ...
            it 'should have rdf triples' do
              ...
              expect(faldo_nodes.any? do |faldo_node|
                subject.include?(RDF::Statement.new(faldo_node, Faldo.position, 17492)) &&
                    subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.11')))
              end
              ).to eq(true)
            end
          end
          ...
          context 'for rs672601345.vcf' do
            ...
            it 'should have rdf triples' do
              ...
              expect(faldo_nodes.any? do |faldo_node|
                subject.include?(RDF::Statement.new(faldo_node, Faldo.position, 949696)) &&
                    subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10')))
              end
              ).to eq(true)
              ...
              it 'should have rdf triples' do
                ...
                expect(faldo_nodes.any? do |faldo_node|
                  ...
                  subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.Region)) &&
                      subject.include?(RDF::Statement.new(faldo_node, Faldo.begin, begin_subject)) &&
                      subject.include?(RDF::Statement.new(begin_subject, RDF::type, Faldo.ExactPosition)) &&
                      subject.include?(RDF::Statement.new(begin_subject, Faldo.position, 1273412)) &&
                      subject.include?(RDF::Statement.new(begin_subject, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10'))) &&
                      subject.include?(RDF::Statement.new(faldo_node, Faldo.end, end_subject)) &&
                      subject.include?(RDF::Statement.new(end_subject, RDF::type, Faldo.ExactPosition)) &&
                      subject.include?(RDF::Statement.new(end_subject, Faldo.position, 1273420)) &&
                      subject.include?(RDF::Statement.new(end_subject, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10')))
                end
                ).to eq(true)
              end
            ...
            context 'for rs553355578.vcf' do
              ...
              context 'for A-C' do
                ...
                it 'should have rdf triples' do
                  ...
                  faldo_nodes = subject.select do |statement|
                    statement.subject == refsnp_part_uri && statement.predicate == Faldo.location
                  end.map(&:object)

                  expect(faldo_nodes.count).to eq(1)

                  expect(faldo_nodes.any? do |faldo_node|
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.position, 133198)) &&
                        subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.ExactPosition)) &&
                        subject.include?(RDF::Statement.new(faldo_node, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10')))
                  end
                  ).to eq(true)
              ...
              context 'for A-G' do
                ...
                it 'should have rdf triples' do
                  ...
                  frequency_nodes = subject.select do |statement|
                    statement.subject == refsnp_part_uri && statement.predicate == SNPO.frequency
                  end.map(&:object)
                  ...
                  is_expected.to include(RDF::Statement.new(frequency_node, RDF::type, SNPO.Frequency))

                  is_expected.to include(RDF::Statement.new(frequency_node, RDF::type, SNPO.send('1000GenomesFrequency')))
          ...
          context 'for rs539283387.vcf' do
            ...
            it 'should have rdf triples' do
              ...
              frequency_nodes = subject.select do |statement|
                statement.subject == refsnp_uri && statement.predicate == SNPO.frequency
              end.map(&:object)

              expect(frequency_nodes.count).to eq(4)

              expect(frequency_nodes.any? do |frequency_node|
                subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.Frequency)) &&
                    subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.send('1000GenomesFrequency'))) &&
                    subject.include?(RDF::Statement.new(frequency_node, RDF::value, 0.009585))
              end).to eq(true)

              expect(frequency_nodes.any? do |frequency_node|
                subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.Frequency)) &&
                    subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.ExacFrequency)) &&
                    subject.include?(RDF::Statement.new(frequency_node, RDF::value, 0.001141))
              end).to eq(true)

              expect(frequency_nodes.any? do |frequency_node|
                subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.Frequency)) &&
                    subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.GnomadFrequency)) &&
                    subject.include?(RDF::Statement.new(frequency_node, RDF::value, 0.01169))
              end).to eq(true)

              expect(frequency_nodes.any? do |frequency_node|
                subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.Frequency)) &&
                    subject.include?(RDF::Statement.new(frequency_node, RDF::type, SNPO.TopmedFrequency)) &&
              ...
              expect(faldo_nodes.any? do |faldo_node|
                subject.include?(RDF::Statement.new(faldo_node, Faldo.position, 955563)) &&
                    subject.include?(RDF::Statement.new(faldo_node, RDF::type, Faldo.ExactPosition)) &&
                    subject.include?(RDF::Statement.new(faldo_node, Faldo.reference, RDF::URI.new(PREFIXES[:refseq] + 'NC_000001.10')))
              end
              ).to eq(true)
